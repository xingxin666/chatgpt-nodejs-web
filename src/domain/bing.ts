...
export async function replyBing(prompt, res, options, systemMessage) {
  try {
    ...
    if (bingSocket) {
      ...
      return new Promise((resolve) => {
        bingSocket.on('init:finish', () => { // socket初始化完成
          ...
          res.write(`\n${JSON.stringify({ text: `开始查询：${prompt}` })}`)
          ...
          if (!myChat)
            ...
            console.warn('bingSocket data返回结果: ',data)
          bingSocket.clearWs()
          resolve(myChat)
        })
      })
